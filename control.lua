...
local pm_deps = {State=State, GUI=GUI, Constants=Constants, Util=Util, Chuansongmen=Chuansongmen, log_debug=log_debug}
...
log_debug("传送门 DEBUG (find_portal_path_for_cybersyn): 正在查找从地表 " .. source_surface_index .. " 到 " .. destination_surface_index .. " 的传送门路径...")
...
local se_direction = (struct.direction == defines.direction.east or struct.direction == defines.direction.south) and defines.direction.east or defines.direction.west
...
local gps_tag_A = "[gps=" .. struct_A.position.x .. "," .. struct_A.position.y .. "," .. struct_A.surface.name .. "]"
for _, player in pairs(game.players) do
  if settings.get_player_settings(player)["chuansongmen-show-power-warnings"].value == true then
player.print({"messages.chuansongmen-warning-power-disconnected-shards", gps_tag_A, struct_A.name, struct_B.name})
end
end
else
  -- 【续期成功】
...
if count_B > 0 then inv_B.remove({name = "chuansongmen-spacetime-shard", count = 1}) else inv_A.remove({name = "chuansongmen-spacetime-shard", count = 1}) end
  ...
  for _, player in pairs(game.players) do
if settings.get_player_settings(player)["chuansongmen-show-power-warnings"].value == true then
  player.print({"messages.chuansongmen-info-power-reconnected-auto", gps_tag_A, struct_A.name, struct_B.name})
  end
  end
  end
  end
...
log_debug("传送门 DEBUG (event): on_trigger_created_entity 捕捉到碰撞检测实体 '" .. Constants.name_train_collision_detector .. "' 创建！")
