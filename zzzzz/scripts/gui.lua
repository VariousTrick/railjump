...
function GUI.create_cybersyn_anchor_gui(player, entity)
	...
	if left_gui[anchor_frame_name] and left_gui[anchor_frame_name].valid then
		return
	end
...
function GUI.build_or_update(player, entity)
	log_debug(
		"传送门 GUI (build_or_update): 开始为玩家 "
			.. player.name
			.. " 构建/更新 GUI, 实体 unit_number: "
			.. entity.unit_number
	)
	...
	if storage.chuansongmen_player_settings == nil then
		storage.chuansongmen_player_settings = {}
	end
	if storage.chuansongmen_player_settings[player.index] == nil then
		storage.chuansongmen_player_settings[player.index] = {}
	end
	if storage.chuansongmen_player_settings[player.index].show_preview == nil then
		local default = player.mod_settings["chuansongmen_show_preview"]
			and player.mod_settings["chuansongmen_show_preview"].value
	...
	if gui.chuansongmen_main_frame then
		gui.chuansongmen_main_frame.destroy()
	end
	...
	local frame = gui.add({
		type = "frame",
		name = "chuansongmen_main_frame",
		direction = "vertical",
		anchor = anchor,
		tags = { unit_number_str = tostring(entity.unit_number) },
	})
	...
	if my_data.paired_to_id then
		...
		pair_status_flow.add({
			type = "label",
			caption = "已连接到 " .. (opposite and opposite.name or "未知") .. " (ID: " .. tostring(
	...
	for _, data in pairs(MOD_DATA.portals) do
		...
		if
			data.id ~= my_data.id
			and data.entity
			and data.entity.valid
			and data.entity.direction == my_data.entity.direction
			and (not data.paired_to_id or data.paired_to_id == my_data.id)
		then
			-- 1. 准备富文本名称
			local icon_prefix = ""
			...
			-- 2. 构建本地化条目
			local item_entry = {
				"gui.chuansongmen-dropdown-item-format",
				rich_name,
				tostring(data.id),
				data.entity.surface.name,
			}
	...
	if selected_idx_to_set > 0 then
		dropdown.selected_index = selected_idx_to_set
	end
	...
	if my_data.paired_to_id then
		...
		if my_data.power_connection_status == "connected" then
			local power_button = button_flow.add({
				type = "button",
				name = "disconnect_power_button",
				caption = { "gui.chuansongmen-button-disconnect-power" },
			})
			...
			local power_button = button_flow.add({
				type = "button",
				name = "connect_power_button",
				caption = { "gui.chuansongmen-button-connect-power" },
			})
		...
		local pair_button =
			button_flow.add({ type = "button", name = "pair_button", caption = { "gui.chuansongmen-button-pair" } })
	...
	teleport_flow.add({
		type = "button",
		name = "player_teleport_button",
		caption = { "gui.chuansongmen-button-player-teleport" },
	})
	local view_caption = (remote.interfaces["space-exploration"] and { "gui.chuansongmen-button-remote-view" })
		or { "gui.chuansongmen-button-map-view" }
	...
	if my_data.paired_to_id and player_settings.show_preview then
		...
		if opposite and opposite.entity and opposite.entity.valid then
			frame.add({
				type = "label",
				style = "frame_title",
				caption = "远程预览: " .. opposite.name .. " [" .. opposite.entity.surface.name .. "]",
			}).style.left_padding =
				8
			...
			local camera = preview_frame.add({
				type = "camera",
				name = "preview_camera",
				position = opposite.entity.position,
				zoom = 0.15,
...
function GUI.open_fullscreen_camera_view(player, target_struct)
	log_debug(
		"传送门 GUI (open_fullscreen_camera_view): 为玩家 "
			.. player.name
			.. " 打开全屏观察, 目标: "
			.. target_struct.name
			.. " (ID: "
	...
	title_flow.add({
		type = "button",
		name = "chuansongmen_close_fullscreen_view_button",
		caption = { "gui.chuansongmen-button-close-fullscreen-view" },
		style = "red_button",
	})

	local camera = frame.add({
		type = "camera",
		name = "chuansongmen_fullscreen_camera",
		position = target_struct.position,
		surface_index = target_struct.surface.index,
		zoom = 0.25,
	})
...
function GUI.handle_click(event)
	...
	elseif element_name == "player_teleport_button" then
		if not MOD_DATA.players_to_teleport then
	...
	elseif element_name == "se_remote_view_button" then
		...
		if remote.interfaces["space-exploration"] and remote.interfaces["space-exploration"]["remote_view_start"] then
			...
			log_debug(
				"传送门 GUI (handle_click): 玩家 "
					.. player.name
					.. " 正在使用 2.0 API (set_controller) 进行地图观察。缩放级别: "
					.. current_zoom
			)
