...
function TeleportHandler.teleport_next(struct)
	...
	if can_place and (carriage_ahead or is_clear) then
		...
		if not carriage_ahead then
			...
			log_debug(
				"传送门 DEBUG (teleport_next): [状态保护] 已强制第一节新车保持手动模式，防止早产。"
			)
...
function TeleportHandler.check_carriage_at_location(surface, position)
	...
	-- [优化] 直接向引擎查询该坐标下的传送门实体
	-- 因为爆炸发生在传送门碰撞盒内部，所以一定能找到它
	local found_entities = surface.find_entities_filtered({
		position = position,
		name = Constants.name_entity, -- "chuansongmen-entity"
		limit = 1,
	})

	if not (found_entities and found_entities[1]) then
		-- 理论上不应该发生，除非爆炸偏移到了传送门外面
		return
	end

	local portal_entity = found_entities[1]
	local struct = State.get_struct(portal_entity)

	-- 如果找到了有效的数据结构，直接执行检测逻辑
	if
		struct
		and struct.entity
		...
		-- 这里不再需要 Util.position_in_rect，因为 find_entities_filtered 已经确认了位置
	then
...
function TeleportHandler.hypertrain_manage_speed(struct)
	...
	then
		...
		local should_push = train_ahead.manual_mode
			or train_ahead.state == defines.train_state.on_the_path
			or train_ahead.state == defines.train_state.no_path
