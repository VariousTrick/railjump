...
function PortalManager.setup_internal_entities(struct)
	log_debug(
		"传送门 DEBUG (setup_internal_entities): Function called. Struct ID: " .. (struct and struct.id or "N/A")
	)
	log_debug(
		"传送门 DEBUG (setup_internal_entities): 开始为传送门 ID: "
			.. struct.id
			.. " 收集内部实体定义..."
	)
	...
	log_debug(
		"传送门 DEBUG (setup_internal_entities): 实体方向: "
			.. struct.direction
			.. ", 映射到内部逻辑方向: "
			.. (se_direction == defines.direction.east and "east" or "west")
	)

	if not Constants.internals[se_direction] then
		log_debug(
			"传送门 致命错误 (setup_internal_entities): 无法在 Constants.internals 中找到方向 "
				.. tostring(se_direction)
				.. " 的定义！"
		)
	...
	log_debug(
		"传送门 DEBUG (setup_internal_entities): 收集完毕，总计 "
			.. #entities_to_create
			.. " 个内部实体定义。"
	)
	...
	for i, creation_data in ipairs(entities_to_create) do
		log_debug(
			"传送门 DEBUG (setup_internal_entities): 正在创建 ["
				.. i
				.. "/"
				.. #entities_to_create
				.. "]: "
		...
		if not sub_entity then
			log_debug(
				"传送门 致命错误 (setup_internal_entities): create_entity 失败！原型: "
					.. creation_data.name
					.. " 位置: "
					.. serpent.line(creation_data.position)
			)
			...
			if sub_entity.type == "train-stop" and sub_entity.name == "chuansongmen-train-stop" then
				if Chuansongmen.DEBUG_MODE_ENABLED then
					log_debug(
						"传送门 DEBUG (setup_internal_entities): 已创建 [train-stop], 存入 struct.station"
					)
				end
			...
			elseif sub_entity.type == "electric-pole" and sub_entity.name == "chuansongmen-energy-pole" then
				log_debug(
					"传送门 DEBUG (setup_internal_entities): 已创建 [electric-pole], 存入 struct.electric_pole"
				)
				struct.electric_pole = sub_entity
			elseif sub_entity.type == "power-switch" and sub_entity.name == "chuansongmen-power-switch" then
				log_debug(
					"传送门 DEBUG (setup_internal_entities): 已创建 [power-switch], 存入 struct.power_switch"
				)
	...
	log_debug(
		"传送门 DEBUG (setup_internal_entities): 普通内部实体 (轨道, 灯, 信号灯等) 创建完毕, 总数: "
			.. #sub_entities
	)
	...
	if not collider_pos_offset then
		log_debug(
			"传送门 致命错误 (setup_internal_entities): 无法找到方向 "
				.. tostring(se_direction)
				.. " 的碰撞器位置 (collider_position)！"
		)
	else
		if Chuansongmen.DEBUG_MODE_ENABLED then
	...
	if watch_rect then
		struct.watch_area = {
			...
			right_bottom = Util.vectors_add(struct.position, watch_rect.right_bottom),
		}
		if Chuansongmen.DEBUG_MODE_ENABLED then
		...
		log_debug(
			"传送门 错误 (setup_internal_entities): 无法设置 watch_area, 方向 "
				.. tostring(se_direction)
				.. " 定义缺失。"
		)
	end
	...
	if output_rect then
		struct.output_area = {
			...
			right_bottom = Util.vectors_add(struct.position, output_rect.right_bottom),
		}
		if Chuansongmen.DEBUG_MODE_ENABLED then
		...
		log_debug(
			"传送门 错误 (setup_internal_entities): 无法设置 output_area, 方向 "
				.. tostring(se_direction)
				.. " 定义缺失。"
		)
	end
...
function PortalManager.on_built(entity)
	...
	log_debug(
		"传送门 DEBUG (on_built): 传送门被建造。新传送门 ID: "
			.. id
			.. ", 实体 unit_number: "
			.. entity.unit_number
			.. ", 名称: "
	...
	if
		not struct.station
		or not struct.power_switch
		or not struct.electric_pole
		or not struct.energy_interface_actual
	then
		log_debug(
			"传送门 致命错误 (on_built): setup_internal_entities 未能设置关键内部实体！传送门ID: "
				.. id
		)
		if not struct.station then
			if Chuansongmen.DEBUG_MODE_ENABLED then
				log_debug("错误详情: station 为 nil")
			end
		end
		if not struct.power_switch then
			if Chuansongmen.DEBUG_MODE_ENABLED then
				log_debug("错误详情: power_switch 为 nil")
			end
		end
		if not struct.electric_pole then
			if Chuansongmen.DEBUG_MODE_ENABLED then
				log_debug("错误详情: electric_pole 为 nil")
			end
		end
		if not struct.energy_interface_actual then
			if Chuansongmen.DEBUG_MODE_ENABLED then
				log_debug("错误详情: energy_interface_actual 为 nil")
			end
		end
		local player = game.get_player(entity.last_user)
		if player then
			player.print({ "messages.chuansongmen-error-internal-creation-failed" })
		end
	end
	...
	-- 【新增】将新传送门的 unit_number 有序地插入到对应的时间桶中
	local bucket_index = struct.unit_number % 60
	if MOD_DATA.portal_buckets and MOD_DATA.portal_buckets[bucket_index] then
		local bucket = MOD_DATA.portal_buckets[bucket_index]
		table.insert(bucket, struct.unit_number)
		table.sort(bucket) -- 简单起见，直接排序。对于低频事件，性能足够。
		log_debug(
			"传送门 DEBUG (on_built): 传送门 unit_number "
				.. struct.unit_number
				.. " 已加入时间桶 "
				.. bucket_index
		)
...
function PortalManager.on_mined(entity)
	...
	if my_data then
		-- 【新增】从其所属的时间桶中移除
		local bucket_index = my_data.unit_number % 60
		if MOD_DATA.portal_buckets and MOD_DATA.portal_buckets[bucket_index] then
			local bucket = MOD_DATA.portal_buckets[bucket_index]
			for i, v in ipairs(bucket) do
				if v == my_data.unit_number then
					table.remove(bucket, i)
					log_debug(
						"传送门 DEBUG (on_mined): 传送门 unit_number "
							.. my_data.unit_number
							.. " 已从时间桶 "
							.. bucket_index
							.. " 移除。"
		...
		if Chuansongmen.DEBUG_MODE_ENABLED then
			log_debug(
				"传送门 DEBUG (on_mined): 找到传送门数据, ID: " .. my_data.id .. ". 开始清理流程..."
			)
		end
		if my_data.paired_to_id then
			...
			if opposite then
				...
				log_debug(
					"传送门 DEBUG (on_mined): 已通知对侧传送门 (ID: "
						.. opposite.id
						.. ") 解除传送配对。"
				)
				for _, p in pairs(game.players) do
					if p.opened == opposite.entity then
						GUI.build_or_update(p, opposite.entity)
					end
				end
			else
				log_debug(
					"传送门 警告 (on_mined): 尝试通知对侧传送门 (ID: "
						.. my_data.paired_to_id
						.. ") 解除配对，但找不到有效的对侧结构。"
				)
			end
		...
		if my_data.power_partner_id then
			...
			if power_partner then
				local primary_struct, secondary_struct = my_data.is_power_primary and { my_data, power_partner }
					or { power_partner, my_data }
				...
				log_debug(
					"传送门 DEBUG (on_mined): 已通知电网伙伴 (ID: " .. power_partner.id .. ") 断开连接。"
				)
				for _, p in pairs(game.players) do
					if p.opened == power_partner.entity then
						GUI.build_or_update(p, power_partner.entity)
					end
				end
			else
				log_debug(
					"传送门 警告 (on_mined): 找不到有效的电网伙伴 (ID: "
						.. my_data.power_partner_id
						.. ") 来断开连接。"
				)
			end
		...
		if my_data.station and my_data.station.valid then
			my_data.station.destroy()
		end
		if my_data.power_switch and my_data.power_switch.valid then
			my_data.power_switch.destroy()
		end
		if my_data.electric_pole and my_data.electric_pole.valid then
			my_data.electric_pole.destroy()
		end
		if my_data.energy_interface_actual and my_data.energy_interface_actual.valid then
			my_data.energy_interface_actual.destroy()
		end
		if my_data.collider and my_data.collider.valid then
			my_data.collider.destroy()
		end
		if my_data.sub_entities then
			log_debug(
				"传送门 DEBUG (on_mined): 正在销毁 "
					.. #my_data.sub_entities
					.. " 个 sub_entities (轨道, 灯等)..."
			)
		...
		log_debug(
			"传送门 DEBUG (on_mined): 传送门 ID "
				.. my_data.id
				.. " 的数据已从全局表移除。清理流程结束。"
		)
	else
		log_debug(
			"传送门 警告 (on_mined): 尝试清理 unit_number 为 "
				.. entity.unit_number
				.. " 的传送门，但在全局表中未找到其数据。"
		)
	end
...
function PortalManager.connect_wires(primary_struct, secondary_struct)
	log_debug(
		"传送门 DEBUG (connect_wires): 正在连接电线: 主控ID "
			.. primary_struct.id
			.. " <-> 次控ID "
			.. secondary_struct.id
	)
	if
		primary_struct.power_switch
		and primary_struct.electric_pole
		and secondary_struct.electric_pole
		and primary_struct.power_switch.valid
		and primary_struct.electric_pole.valid
		and secondary_struct.electric_pole.valid
	then
		if Chuansongmen.DEBUG_MODE_ENABLED then
		...
		primary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.circuit_red, true).connect_to(
			secondary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.circuit_red, true),
			false,
			defines.wire_origin.script
		)
		primary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.circuit_green, true).connect_to(
			secondary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.circuit_green, true),
			false,
			defines.wire_origin.script
		)
		if Chuansongmen.DEBUG_MODE_ENABLED then
		...
		if not primary_struct.power_switch then
			if Chuansongmen.DEBUG_MODE_ENABLED then
				log_debug("错误详情: 主控 (ID " .. primary_struct.id .. ") 缺少 power_switch")
			end
		end
		if not primary_struct.electric_pole then
			if Chuansongmen.DEBUG_MODE_ENABLED then
				log_debug("错误详情: 主控 (ID " .. primary_struct.id .. ") 缺少 electric_pole")
			end
		end
		if not secondary_struct.electric_pole then
			if Chuansongmen.DEBUG_MODE_ENABLED then
				log_debug("错误详情: 次控 (ID " .. secondary_struct.id .. ") 缺少 electric_pole")
			end
		end
	end
end
...
function PortalManager.disconnect_wires(primary_struct, secondary_struct)
	log_debug(
		"传送门 DEBUG (disconnect_wires): 正在断开电线: 主控ID "
			.. (primary_struct and primary_struct.id or "nil")
			.. ", 次控ID "
			.. (secondary_struct and secondary_struct.id or "nil")
	)
	if primary_struct and primary_struct.power_switch and primary_struct.power_switch.valid then
		...
		-- 断开开关两侧的铜线连接
		primary_struct.power_switch
			.get_wire_connector(defines.wire_connector_id.power_switch_left_copper, true)
			.disconnect_all(defines.wire_origin.script)
		primary_struct.power_switch
			.get_wire_connector(defines.wire_connector_id.power_switch_right_copper, true)
			.disconnect_all(defines.wire_origin.script)
		log_debug(
			"传送门 DEBUG (disconnect_wires): 主控 (ID "
				.. primary_struct.id
				.. ") 的电源开关铜线已断开。"
		)
	else
		log_debug(
			"传送门 警告 (disconnect_wires): 尝试断开电线，但找不到有效的主控电源开关。"
		)
	end
end
...
function PortalManager.pair_portals(player_index, portal_id, target_id)
	...
	log_debug(
		"传送门 DEBUG (remote): 收到 pair_portals 调用, 玩家: "
	...
	if not (player and portal_id and target_id) then
		if Chuansongmen.DEBUG_MODE_ENABLED then
			log_debug("传送门 错误 (remote pair_portals): 参数无效。")
		end
		return
	end
	...
	if not (my_data and my_data.entity and my_data.entity.valid) then
		player.print({ "messages.chuansongmen-error-self-invalid" })
		return
	end
	if not (target_struct and target_struct.entity and target_struct.entity.valid) then
		player.print({ "messages.chuansongmen-error-target-invalid" })
		return
	end

	local is_compatible, reason =
		Chuansongmen.are_directions_compatible(my_data.entity.direction, target_struct.entity.direction)
	if not is_compatible then
		player.print({ "messages.chuansongmen-error-direction-mismatch-rich" })
		log_debug(
			"传送门 警告 (pair_portals): ID "
				.. my_data.id
				.. " 和 ID "
				.. target_struct.id
				.. " 方向不兼容 ("
	...
	if target_struct.paired_to_id then
		player.print({ "messages.chuansongmen-error-target-already-paired" })
		return
	end
	if my_data.paired_to_id then
		player.print({ "messages.chuansongmen-error-self-already-paired" })
		return
	end
	...
	if not resource_cost_enabled then
		...
		if Chuansongmen.DEBUG_MODE_ENABLED then
			log_debug(
				"传送门 DEBUG (remote pair_portals): 检测到 [无消耗] 模式，正在自动连接电网..."
			)
		end
		...
		log_debug(
			"传送门 DEBUG (remote pair_portals): 电网状态已更新 (power_connection_status = 'connected')。主控 ID: "
				.. primary_struct.id
		)
		...
		if Chuansongmen.DEBUG_MODE_ENABLED then
			log_debug(
				"传送门 DEBUG (remote pair_portals): 检测到 [有消耗] 模式，电网等待手动连接。"
			)
		end
	...
	for _, p in pairs(game.players) do
		if p.opened == target_struct.entity then
			GUI.build_or_update(p, target_struct.entity)
		end
		if p.opened == my_data.entity and p ~= player then
...
function PortalManager.unpair_portals(player_index, portal_id)
	local player = game.get_player(player_index)
	log_debug(
		"传送门 DEBUG (remote): 收到 unpair_portals 调用, 玩家: "
			.. (player and player.name or "未知")
			.. ", portal_id: "
			.. tostring(portal_id)
	)
	if not (player and portal_id) then
		return
	end
	...
	if my_data.paired_to_id then
		if Chuansongmen.DEBUG_MODE_ENABLED then
			log_debug(
				"传送门 DEBUG (remote unpair_portals): 正在解除传送门 ID " .. my_data.id .. " 的配对..."
			)
		end
		local opposite = State.get_opposite_struct(my_data)
		if opposite then
			opposite.paired_to_id = nil
			log_debug(
				"传送门 DEBUG (remote unpair_portals): 对侧传送门 (ID: "
					.. opposite.id
					.. ") 的配对已清除。"
			)
		end
		...
		if my_data.power_connected then
			log_debug(
				"传送门 DEBUG (remote unpair_portals): 检测到电网已连接 (power_connected=true)，正在断开..."
			)
			...
			if power_partner then
				...
				log_debug(
					"传送门 DEBUG (remote unpair_portals): 对侧电网状态已更新 (power_connected=false)。ID: "
						.. power_partner.id
				)
			end
			my_data.power_connected = false
			...
			if Chuansongmen.DEBUG_MODE_ENABLED then
				log_debug(
					"传送门 DEBUG (remote unpair_portals): 本地电网状态已更新 (power_connected=false)。"
				)
			end
		end
		-- =======================================================
		...
		log_debug(
			"传送门 DEBUG (remote unpair_portals): 传送门 ID "
				.. my_data.id
				.. " 本身未配对，无需操作。"
		)
...
function PortalManager.update_portal_details(player_index, portal_id, new_name_with_icon)
	...
	if icon_type and icon_name then
		...
		log_debug(
			"传送门 PortalManager: 解析成功。新图标: {type='"
				.. icon_type
				.. "', name='"
				.. icon_name
				.. "'}, 新名称: '"
		...
		log_debug(
			"传送门 PortalManager: 解析为纯文本。新名称: '" .. my_data.name .. "' (图标保持不变)"
		)
	end
...
function PortalManager.connect_portal_power(player, portal_id)
	log_debug(
		"传送门 DEBUG (connect_portal_power): 玩家 "
			.. player.name
			.. " 请求连接电网，传送门ID: "
			.. portal_id
	)
	...
	if not opposite then
		player.print({ "messages.chuansongmen-error-no-opposite-found" })
		return
	end
	if my_data.power_connection_status == "connected" then
		player.print({ "messages.chuansongmen-info-power-already-connected" })
		return
	end
	...
	if (count_A + count_B) < 2 then
		...
		log_debug(
			"传送门 DEBUG (connect_portal_power): 连接失败，网络中碎片总数 ("
				.. (count_A + count_B)
				.. ") 不足2个。"
		)
		return
	...
	log_debug(
		"传送门 DEBUG (connect_portal_power): 电网连接成功。ID: "
			.. my_data.id
			.. " <-> "
			.. opposite.id
			.. "。服务到期tick: "
...
function PortalManager.disconnect_portal_power(player, portal_id)
	...
	if my_data.power_connection_status == "disconnected" then
		if player then
			player.print({ "messages.chuansongmen-info-power-already-disconnected" })
		end
		return
	end
	...
	if player then
		...
		if Chuansongmen.DEBUG_MODE_ENABLED then
			log_debug(
				"传送门 DEBUG (disconnect_portal_power): 玩家手动断开，状态设置为 'disconnected'。"
			)
		end
	else
		...
		log_debug(
			"传送门 DEBUG (disconnect_portal_power): 系统自动断开，状态设置为 'disconnected_by_system'。"
		)
	end
	...
	log_debug(
		"传送门 DEBUG (disconnect_portal_power): 电网断开成功。ID: "
			.. my_data.id
			.. " <-> "
			.. opposite.id
			.. "。状态已重置。"
