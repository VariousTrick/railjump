...
return settings.startup["chuansongmen-enable-resource-cost"] and settings.startup["chuansongmen-enable-resource-cost"].value or false
end
...
local default = player.mod_settings["chuansongmen_show_preview"] and player.mod_settings["chuansongmen_show_preview"].value
...
title_caption = "[" .. my_data.icon.type .. "=" .. my_data.icon.name .. "] " .. my_data.name .. " (ID: " .. my_data.id .. ")"
else
-- 如果没有图标，则使用旧格式
title_caption = "传送门 " .. my_data.name .. " (ID: " .. my_data.id .. ")"
end
frame.add({type = "flow", name = "title_flow"}).add({type = "label", name = "title", caption = title_caption, style = "frame_title", ignored_by_interaction = true})
...
pair_status_flow.add({type = "label", caption = "已连接到 " .. (opposite and opposite.name or "未知") .. " (ID: " .. tostring(my_data.paired_to_id) .. ") [" .. (opposite and opposite.entity and opposite.entity.surface.name or "未知地表") .. "]", style = "bold_label"})
else
...
local display_name = icon_prefix .. data.name .. " (ID: " .. tostring(data.id) .. ") [" .. data.entity.surface.name .. "]"
...
display_name = display_name .. " [已配对: " .. (partner and (partner_icon_prefix .. partner.name) or "未知") .. "]"
end
...
options_flow.add({type = "checkbox", name = "chuansongmen_preview_checkbox", state = player_settings.show_preview, caption = {"mod-setting-name.chuansongmen_show_preview"}, handler = "on_gui_checked_state_changed"})
...
local view_caption = (remote.interfaces["space-exploration"] and {"gui.chuansongmen-button-remote-view"}) or {"gui.chuansongmen-button-map-view"}
...
local camera = preview_frame.add({type="camera", name="preview_camera", position=opposite.entity.position, zoom=0.15, surface_index=opposite.entity.surface.index})
...
log_debug("传送门 GUI (open_fullscreen_camera_view): 为玩家 " .. player.name .. " 打开全屏观察, 目标: " .. target_struct.name .. " (ID: " .. target_struct.id .. ")")
...
local frame = screen_gui.add({ type="frame", name="chuansongmen_fullscreen_view_frame", direction="vertical", style="frame" })
...
if target_id then remote.call("zchuansongmen", "pair_portals", player.index, my_data.id, target_id)
else player.print({"messages.chuansongmen-error-no-target"}) end
else player.print({"messages.chuansongmen-error-select-first"}) end
...
player.print({"messages.chuansongmen-error-invalid-target-simple"}) return
end
...
local zone_info = remote.call("space-exploration", "get_zone_from_surface_index", {surface_index = opposite.entity.surface.index})
if zone_info and zone_info.name then
remote.call("space-exploration", "remote_view_start", {player=player, zone_name=zone_info.name, position=opposite.entity.position})
player.opened = nil
end
else player.print({"messages.chuansongmen-error-se-tech-required"}) end
else
GUI.open_fullscreen_camera_view(player, opposite)
player.opened = nil
end
