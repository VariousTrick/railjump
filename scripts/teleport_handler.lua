...
if log_debug then
  ...
  log_debug("传送门 DEBUG (carriage_transfer_contents): 正在从 " .. carriage.name .. " (旧 unit_number: " .. carriage.unit_number .. ") 复制内容到 " .. new_carriage.name .. " (新 unit_number: " .. new_carriage.unit_number .. ")")
...
if final_train and final_train.valid then
  ...
  local speed_direction = Chuansongmen.elevator_east_sign(opposite_struct) * Chuansongmen.carriage_east_sign(opposite_struct.carriage_ahead) * Chuansongmen.train_forward_sign(opposite_struct.carriage_ahead)
  ...
  log_debug("传送门 DEBUG (finish_teleport): 状态恢复完毕。模式: " .. (final_train.manual_mode and "手动" or "自动") .. ", 速度: " .. final_train.speed)
  else
log_debug("传送门 警告 (finish_teleport): 找不到有效的最终火车进行状态恢复。")
end
struct.carriage_behind, struct.carriage_ahead, opposite_struct.carriage_behind, opposite_struct.carriage_ahead = nil, nil, nil, nil
...
if SE_TELEPORT_FINISHED_EVENT_ID and final_train and final_train.valid and opposite_struct.old_train_id then
  log_debug("传送门 SE 兼容: 正在为新火车 ID " .. final_train.id .. " (旧火车ID: " .. opposite_struct.old_train_id .. ") 触发 on_train_teleport_finished 事件。")
...
if not (carriage.train and carriage.train.valid) then
  ...
  local se_direction = (opposite.direction == defines.direction.east or opposite.direction == defines.direction.south) and defines.direction.east or defines.direction.west
  ...
  local is_clear = opposite.surface.count_entities_filtered{type=Constants.stock_types, area=opposite.output_area, limit=1} == 0
...
if not carriage_ahead then
  opposite.carriage_ahead_manual_mode, opposite.old_train_speed, opposite.old_train_id = carriage.train.manual_mode, carriage.train.speed, carriage.train.id
...
local speed_dir = Chuansongmen.elevator_east_sign(opposite) * Chuansongmen.carriage_east_sign(new_carriage) * Chuansongmen.train_forward_sign(new_carriage)
...
if final_inv and final_inv.get_item_count("chuansongmen-exotic-matter") == 0 then
  ...
  game.print({"messages.chuansongmen-warning-exotic-matter-depleted-train", producer_portal.name, producer_portal.id})
  end
...
log_debug("传送门 DEBUG (check_carriage): [时刻表修复] 已在下一站插入临时路障站点，并重定向火车目标。")
  ...
  local average_speed = ((train_a.weight * math.abs(train_a.speed)) + (train_b.weight * math.abs(train_b.speed))) / total_weight
...
local speed_direction = Chuansongmen.elevator_east_sign(struct) * Chuansongmen.carriage_east_sign(struct.carriage_behind) * Chuansongmen.train_forward_sign(struct.carriage_behind)
train_behind.speed = passive_train_speed * speed_direction
end
if math.abs(train_ahead.speed) < passive_train_speed then
  local speed_direction = Chuansongmen.elevator_east_sign(struct) * Chuansongmen.carriage_east_sign(struct.carriage_ahead) * Chuansongmen.train_forward_sign(struct.carriage_ahead)
