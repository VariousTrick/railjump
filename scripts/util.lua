...
log_util("!! 警告 (transfer_all_inventories): [主方案失败] 'get_inventories' 调用失败。错误: " .. tostring(inventories_or_error) .. " 将启动 [SE后备方案]...")
end
...
log_util("!! 严重兼容性错误 (transfer_inventory_filters): 尝试调用 'supports_inventory_bar' 时发生崩溃！错误信息: " .. tostring(pcall_result_or_error))
...
local gps_tag = "[gps=" .. opposite_portal.position.x .. "," .. opposite_portal.position.y .. "," .. opposite_portal.surface.name .. "]"
local message = {"messages.chuansongmen-info-remote-consumption", entry_portal.name, gps_tag, opposite_portal.name}
...
-- 3. 如果两边都失败
::fail::
log_util("!! 警告 (consume_shared_resources): 入口 (ID: " .. entry_portal.id .. ") " .. (opposite_portal and ("和对侧 (ID: " .. opposite_portal.id .. ") ") or "") .. "均缺少资源。")
