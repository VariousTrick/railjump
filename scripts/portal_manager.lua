...
log_debug("传送门 DEBUG (setup_internal_entities): 实体方向: " .. struct.direction .. ", 映射到内部逻辑方向: " .. (se_direction == defines.direction.east and "east" or "west"))

if not Constants.internals[se_direction] then
log_debug("传送门 致命错误 (setup_internal_entities): 无法在 Constants.internals 中找到方向 " .. tostring(se_direction) .. " 的定义！")
...
log_debug("传送门 DEBUG (setup_internal_entities): 正在创建 [" .. i .. "/" .. #entities_to_create .. "]: " .. creation_data.name)
local sub_entity = struct.surface.create_entity(creation_data)

if not sub_entity then
log_debug("传送门 致命错误 (setup_internal_entities): create_entity 失败！原型: "..creation_data.name .. " 位置: " .. serpent.line(creation_data.position))
...
log_debug("传送门 DEBUG (setup_internal_entities): 已创建 [energy-interface], 存入 struct.energy_interface_actual")
...
log_debug("传送门 致命错误 (setup_internal_entities): 无法找到方向 " .. tostring(se_direction) .. " 的碰撞器位置 (collider_position)！")
else
...
if watch_rect then
struct.watch_area = { left_top = Util.vectors_add(struct.position, watch_rect.left_top), right_bottom = Util.vectors_add(struct.position, watch_rect.right_bottom) }
...
if output_rect then
struct.output_area = { left_top = Util.vectors_add(struct.position, output_rect.left_top), right_bottom = Util.vectors_add(struct.position, output_rect.right_bottom) }
...
log_debug("传送门 DEBUG (on_built): 传送门被建造。新传送门 ID: " .. id .. ", 实体 unit_number: " .. entity.unit_number .. ", 名称: " .. entity.name)
local struct = {
id = id, name = tostring(id), entity = entity, unit_number = entity.unit_number,
icon = {type = "item", name = "chuansongmen"}, -- 【新增】用于存储传送门的图标, 默认为传送门本身
force_name = entity.force.name, surface = entity.surface, position = entity.position, direction = entity.direction,
paired_to_id = nil, power_partner_id = nil, is_power_primary = false,
-- =======================================================
-- 【电网维持 - 状态机 v2.0】
...
carriage_ahead = nil, carriage_behind = nil, lua_energy = 0, station = nil, power_switch = nil,
electric_pole = nil, energy_interface_actual = nil, collider = nil, sub_entities = {},
watch_area = nil, output_area = nil, carriage_ahead_manual_mode = false, old_train_speed = 0,
carriage_ahead_current_stop = nil, saved_schedule_index = nil
}
...
local primary_struct, secondary_struct = my_data.is_power_primary and {my_data, power_partner} or {power_partner, my_data}
...
if my_data.energy_interface_actual and my_data.energy_interface_actual.valid then my_data.energy_interface_actual.destroy() end
...
primary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.pole_copper, true).connect_to(primary_struct.power_switch.get_wire_connector(defines.wire_connector_id.power_switch_left_copper, true), false, defines.wire_origin.script)
log_debug("传送门 DEBUG (connect_wires): 主控电线杆 -> 主控开关 (左侧铜线) 已连接。")
secondary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.pole_copper, true).connect_to(primary_struct.power_switch.get_wire_connector(defines.wire_connector_id.power_switch_right_copper, true), false, defines.wire_origin.script)
log_debug("传送门 DEBUG (connect_wires): 次控电线杆 -> 主控开关 (右侧铜线) 已连接。")
primary_struct.power_switch.power_switch_state = true
primary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.circuit_red, true).connect_to(secondary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.circuit_red, true), false, defines.wire_origin.script)
primary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.circuit_green, true).connect_to(secondary_struct.electric_pole.get_wire_connector(defines.wire_connector_id.circuit_green, true), false, defines.wire_origin.script)
...
if not secondary_struct.electric_pole then log_debug("错误详情: 次控 (ID " .. secondary_struct.id .. ") 缺少 electric_pole") end
end
end
...
log_debug("传送门 DEBUG (disconnect_wires): 正在断开电线: 主控ID " .. (primary_struct and primary_struct.id or "nil") .. ", 次控ID " .. (secondary_struct and secondary_struct.id or "nil"))
...
-- 断开开关两侧的铜线连接
primary_struct.power_switch.get_wire_connector(defines.wire_connector_id.power_switch_left_copper, true).disconnect_all(defines.wire_origin.script)
primary_struct.power_switch.get_wire_connector(defines.wire_connector_id.power_switch_right_copper, true).disconnect_all(defines.wire_origin.script)
...
log_debug("传送门 DEBUG (remote): 收到 pair_portals 调用, 玩家: " .. (player and player.name or "未知") .. ", portal_id: " .. tostring(portal_id) .. ", target_id: " .. tostring(target_id))
if not (player and portal_id and target_id) then log_debug("传送门 错误 (remote pair_portals): 参数无效。"); return end

local my_data = State.get_struct_by_id(portal_id)
local target_struct = State.get_struct_by_id(target_id)

if not (my_data and my_data.entity and my_data.entity.valid) then player.print({"messages.chuansongmen-error-self-invalid"}); return end
if not (target_struct and target_struct.entity and target_struct.entity.valid) then player.print({"messages.chuansongmen-error-target-invalid"}); return end

local is_compatible, reason = Chuansongmen.are_directions_compatible(my_data.entity.direction, target_struct.entity.direction)
if not is_compatible then
player.print({"messages.chuansongmen-error-direction-mismatch-rich"})
log_debug("传送门 警告 (pair_portals): ID " .. my_data.id .. " 和 ID " .. target_struct.id .. " 方向不兼容 (" .. reason .. ")，配对被取消。")
return
end

if target_struct.paired_to_id then player.print({"messages.chuansongmen-error-target-already-paired"}); return end
if my_data.paired_to_id then player.print({"messages.chuansongmen-error-self-already-paired"}); return end
...
-- 【核心修改】根据游戏模式决定电网连接行为
-- =======================================================
local resource_cost_enabled = settings.startup["chuansongmen-enable-resource-cost"] and settings.startup["chuansongmen-enable-resource-cost"].value or false
...
log_debug("传送门 DEBUG (remote pair_portals): 电网状态已更新 (power_connection_status = 'connected')。主控 ID: " .. primary_struct.id)
...
local player = game.get_player(player_index)
log_debug("传送门 DEBUG (remote): 收到 unpair_portals 调用, 玩家: " .. (player and player.name or "未知") .. ", portal_id: " .. tostring(portal_id))
...
log_debug("传送门 DEBUG (remote unpair_portals): 对侧电网状态已更新 (power_connected=false)。ID: " .. power_partner.id)
end
my_data.power_connected = false
...
log_debug("传送门 PortalManager: 解析成功。新图标: {type='" .. icon_type .. "', name='" .. icon_name .. "'}, 新名称: '" .. plain_name .. "'")
else
...
local opposite = State.get_opposite_struct(my_data)
if not opposite then player.print({"messages.chuansongmen-error-no-opposite-found"}); return end
if my_data.power_connection_status == "connected" then player.print({"messages.chuansongmen-info-power-already-connected"}); return end
...
-- 智能消耗2个碎片
if count_A > 0 then inv_A.remove({name = "chuansongmen-spacetime-shard", count = 1}) else inv_B.remove({name = "chuansongmen-spacetime-shard", count = 1}) end
if count_B > 0 then inv_B.remove({name = "chuansongmen-spacetime-shard", count = 1}) else inv_A.remove({name = "chuansongmen-spacetime-shard", count = 1}) end
...
log_debug("传送门 DEBUG (connect_portal_power): 电网连接成功。ID: " .. my_data.id .. " <-> " .. opposite.id .. "。服务到期tick: " .. expires_at)
