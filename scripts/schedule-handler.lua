...
log_schedule("DEBUG (transfer_schedule v2.0): 开始为新火车 (ID: " .. new_train.id .. ") 转移时刻表，来源旧火车 (ID: " .. old_train.id .. ")。")
...
log_schedule("DEBUG (transfer_schedule): 获取完毕。站点数: " .. #records_old .. ", 中断数: " .. #interrupts .. ", 当前目标索引: " .. current_stop_index)
...
log_schedule("DEBUG (transfer_schedule): 原始目标索引 " .. logical_next_stop_index .. " - 偏移量 " .. index_correction_offset .. " = 最终目标索引 " .. final_target_index)
...
log_schedule("!! 核心操作 (transfer_schedule): 已命令新火车前往最终目标站点索引 " .. final_target_index .. " ('" .. (final_records[final_target_index].station or "轨道站") .. "')。时刻表转移完成！")
end
